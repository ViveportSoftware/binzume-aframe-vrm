...
var BVHLoaderWrapper = class {
  async load(url, avatar, options) {
    ...
    return await new Promise((resolve, reject) => {
      new BVHLoader().load(url, (result) => {
        options.convertBone && this.fixTrackName(result.clip, avatar, result.skeleton.bones), result.clip.tracks = result.clip.tracks.filter((t) => !t.name.match(/position/) || t.name.match(avatar.bones.hips.name)), resolve(result.clip);
      });
    });
  }
  ...
  fixTrackName(clip, avatar, motionBones) {
    let _vec3 = new THREE.Vector3(), motionHipsBone = motionBones.find((b) => b.name == "hips"), motionUpperChestBone = motionBones.find((b) => b.name == "upperChest"), motionHipsHeight = (motionHipsBone?.position.y || 0) * 2.005, vrmHipsY = avatar.bones.hips?.getWorldPosition(_vec3).y, vrmRootY = avatar.model.getWorldPosition(_vec3).y, vrmHipsHeight = Math.abs(vrmHipsY - vrmRootY), hipsPositionScale = motionHipsBone && motionUpperChestBone ? vrmHipsHeight / motionHipsHeight : 0.09;
